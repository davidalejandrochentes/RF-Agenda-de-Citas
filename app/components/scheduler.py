...
def time_slots_view() -> rx.Component:
    """Displays available time slots grouped by morning and afternoon."""

    def time_button(time: str) -> rx.Component:
        """Helper function to create a time slot button."""
        return rx.el.button(
            rx.moment(
                time,
                format="hh:mm A",
            ...
            on_click=lambda: BarberState.select_time(time),
            type="button",
            class_name=rx.cond(
                BarberState.selected_time == time,
    ...
    return rx.el.div(
        ...
        rx.cond(
            ...
            rx.el.div(
                # Morning Slots
                rx.cond(
                    BarberState.morning_times.length() > 0,
                    rx.el.div(
                        rx.el.h5(
                            "Mañana",
                            class_name="font-medium text-gray-600 mb-2 text-center",
                        ),
                        rx.el.div(
                            rx.foreach(
                                BarberState.morning_times,
                                time_button,
                            ),
                ...
                # Afternoon Slots
                rx.cond(
                    BarberState.afternoon_times.length() > 0,
                    rx.el.div(
                        rx.el.div(
